...
useSeoMeta({ title: 'Lead — Panel Internal', robots: 'noindex, nofollow' })
...
const status = ref('')
const { data, pending, refresh } = await useFetch('/api/internal/leads', {
  query: { status },
})
    ...
    <div class="mt-6 overflow-x-auto rounded-2xl border border-primary-100 bg-white/60">
      <table class="w-full min-w-[760px] text-sm">
        ...
        <tbody>
          ...
          >
            <td class="px-5 py-4">
              <NuxtLink :to="`/internal/leads/${lead.id}`" class="font-medium text-primary hover:underline">
                {{ lead.name }}
              </NuxtLink>
